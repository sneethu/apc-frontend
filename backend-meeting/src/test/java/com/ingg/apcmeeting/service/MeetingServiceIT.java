...
public class MeetingServiceIT {
    ...
    public static final String DAILY_SCRUM_UPDATED = "Daily Scrum Updated";
   ...
   private final Meeting meeting1_nov = Meeting.builder()
    ...
    @BeforeEach
    public void setUp() {
        meetingService = new MeetingService(meetingRepository, publisher, nextSequenceService);
    ...
    public void testUpdate(){
        ...
        Mono<Meeting> updated = meetingService.create(meeting1_nov)
                                        .flatMap(meeting -> {
                                            meeting.setTitle(DAILY_SCRUM_UPDATED);
                                            return meetingService.update(meeting);
        });

        StepVerifier
                .create(updated)
                .expectNextMatches(meeting -> meeting.getTitle().equals(DAILY_SCRUM_UPDATED))
                .verifyComplete();
    }
}
